...
def home():
    if 'loggedin' in session:
        ...
        if is_admin:
            ...
            return render_template('admin.html', audit_logs=audit_logs, username=session['username'])
        else:
...
def admin():
    try:
        ...
        print("Audit Data:", audit)  # Check if data is fetched
...
def reset(reset_token):
                    ...
                    for row in encrypted_email_data:
                        if email == row['email']:
                            ...
                            try:
                                ...
                                msg = 'An error occurred while updating the password. Please try again later.'
                else:
    ...
    return render_template('reset.html', msg=msg, reset_token=reset_token, email=email, password=hashpwd.decode())
